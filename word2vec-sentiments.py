...
for k in range(2,3) :
    ...
    sources = {path+'Test_'+str(k)+'_FALSE.txt':'TEST_NEG',
    ...
    # Se voglio creare il modello e non caricarne uno precedente __________________________________________________
    if load_model == 0:
    #    log.info('TaggedDocument')


        # log.info('D2V')
        model = Doc2Vec(min_count=1, window=10, size=size, sample=1e-4, negative=5, workers=cores)
        ...
        # log.info('Model Save')
        model.save('./models/FakeNews_model_'+mod+'_'+str(k)+'_'+str(size)+'.d2v')

    # LOADMODEL == 1 - Carico Modello __________________________________________________________________________

    model = Doc2Vec.load('./models/FakeNews_model_'+mod+'_'+str(k+1)+'_'+str(size)+'.d2v')


    log.info('Sentiment')
    train_arrays = numpy.zeros((lunghezza*2, size))
    ...
    for i in range(lunghezza):
        ...
        train_arrays[ lunghezza + i] = model.docvecs[prefix_train_neg]
...
# TEST: __________________________________________________________________________________
   ...
   # print lunghezza2
    test_arrays = numpy.zeros((lunghezza2 * 2, size))
    ...
    for j in range(lunghezza2):
        ...
        test_arrays[j] = model.docvecs[prefix_test_pos]
        test_arrays[lunghezza2 + j] = model.docvecs[prefix_test_neg]
...
print('Random Target: %s\n' %(str(sentences.to_array()[doc_id]).split("', u'"))[-1].split("], [")[-1][:-2] )
testo=""
for s in (str(sentences.to_array()[doc_id]).split("TaggedDocument(")[-1]).split("', u'"):
    testo += str(s)+' '
print testo
...
for label, index in [('MOST', 0), ('MEDIAN', len(sims)//2), ('LEAST', len(sims) - 1)]:
    ...
    if p[0] == 'TRAIN':
        if sentiment =='POS':
            indice = len( open(path+'Train_'+str(k)+'_FALSE.txt').read().split('\n')) + indice -1
    else:
        indice += lunghezza*2
        if sentiment =='POS':
            indice = len( open(path+'Test_'+str(k)+'_FALSE.txt').read().split('\n')) + indice -1

    #print indice
    testo=""
    prova = str(sentences.to_array()[indice]).split("TaggedDocument(")[-1]
    check = prova[0:2]
    if check == '[]':
        print "ERROREEEEEEEE"
        # frasi = str(sentences_article.to_array())
        # print frasi
    for s in (str(sentences.to_array()[indice]).split("TaggedDocument(")[-1]).split("', u'"):
       testo += str(s)+' '
    print testo
